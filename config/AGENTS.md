...
# 强制要求
- 代码改动完成后，必须按通用协作规则和项目级开发规范审查改动的代码是否满足对应规则的要求。
- 任何操作完成后，第一，都必须以第三方视角对此次操作进行一次完整的自审与修复操作，不然工作不能算完成。第二，必须检查是否需要同步更新相关的开发规范等事实文档
- 用户提出的问题，都必须先进行举一反三，即审查是否其他地方也存在这个问题或其他潜在没有发现的问题，一并纳入解决范围里。