...
- 公共组件、公共 hooks、store、工具函数等间接影响页面的文件，脚本不能自动分析完整依赖链；需要在 `page-map.json` 中人工维护受影响页面映射。
- 当前页面自动化主要覆盖页面打开、登录态、控制台 error、页面异常、HTTP 500 和白屏检查；暂不覆盖新增、编辑、删除、表单提交等复杂业务流程点击。
- 当前页面自动化不能自动判断页面业务数据是否正确；如列表数据、详情字段、保存结果等业务断言，需要后续补充稳定选择器、业务用例配置和断言逻辑。
